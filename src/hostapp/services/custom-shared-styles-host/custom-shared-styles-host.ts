...
@Injectable()
export class CustomDomSharedStylesHost extends SharedStylesHost implements OnDestroy {
  // Maps all registered host nodes to a list of style nodes that have been added to the host node.
  private readonly styleRef = new Map<string, HTMLStyleElement[]>();
  private hostNodes = new Set<Node>();
  private csp = inject(CSP_NONCE);
  ...
  private addStyleToHost(host: Node, style: string): void {
    ...
    if(this.csp) styleEl.setAttribute('nonce', this.csp);
